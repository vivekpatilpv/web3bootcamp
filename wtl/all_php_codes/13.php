...
if(isset($_POST["id"]) && isset($_POST["name"])  && isset($_POST["location"]) && isset($_POST["department"])  ){
...
$con=mysqli_connect($server,$username,$password);
    ...
    mysqli_query($con, "CREATE TABLE if not exists `$table_name`(id INT(5), name VARCHAR(100), location varchar(100), department varchar(100))");
...
if (!empty($location) && !empty($id) && !empty($name) && !empty($department)) {
$sql="INSERT INTO `$table_name` (`id`, `name`, `location`, `department`) VALUES ('$id', '$name', '$location', '$department')";
...
<div class="container"  style="color: black !important;">  
  ...
  <h1>All Companies</h1>
          ...
          if(isset($_POST['search'])){
            ...
            $sql="SELECT * FROM `$table_name` WHERE `location` LIKE '$search'";
            ...
            if($nrows>0){
              ...
              echo "
             <br> <ul><li> ID: {$row['id']} </li> <li> COMPANY NAME: {$row['name']} </li> <li>CITY: {$row['location']} </li> <li> DEPARTMENT: {$row['department']} </li></ul> </br>"; 
