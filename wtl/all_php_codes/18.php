...
if(isset($_POST["id"]) && isset($_POST["name"])  && isset($_POST["location"]) && isset($_POST["department"]) && isset($_POST["job"])  ){
...
mysqli_query($con, "CREATE TABLE if not exists `$table_name`(id INT(5), name VARCHAR(100), location varchar(100), department varchar(100) ,job varchar(10))");
...
if (!empty($location) && !empty($id) && !empty($name) && !empty($department) && !empty($job)) {
$sql="INSERT INTO `$table_name` (`id`, `name`, `location`, `department`,`job`) VALUES ('$id', '$name', '$location', '$department','$job')";
...
<div class="container"  style="color: black !important;">  
    <form id="contact" action="18.php" method="post">
      ...
      <h4>Fill The Data</h4>
        ...
        <input name="job" placeholder="Job Role" type="text"  required>
  ...
  <h1>All Managers</h1>
          ...
          $sql="SELECT * FROM `$table_name` WHERE `job` ='manager'";
